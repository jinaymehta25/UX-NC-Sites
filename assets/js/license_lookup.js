$(document).ready(function () {
    ...
    dataTableObj = $("#results_table").dataTable({
    ...
    function refresh_tree() {
        ...
        $("#agency-tree").jstree("get_checked", null, true).each
        (function () {
            text = ($(this).find('a').text());
            $.each(find_result(text), function (i, v) {
                $('#results_table').dataTable().fnAddData(v);
            });
        })
    }

    function find_result(text) {
        result = [];
        console.log(text.replace(/^\s+|\s+$/g, ''));
        $.each(results, function (index, value) {
            //   console.log(value[1]);
            console.log(value[1].toLowerCase().replace(/^\s+|\s+$/g, '') + (text.toLowerCase().replace(/^\s+|\s+$/g, '')));
            if (text.toLowerCase().replace(/^\s+|\s+$/g, '').indexOf(value[1].toLowerCase().replace(/^\s+|\s+$/g, '')) >= 0) {
